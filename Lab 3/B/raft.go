...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	// doesn't contain an entry at prevLogIndex
	...
	if len(rf.log)-1 >= args.PrevLogIndex && rf.log[args.PrevLogIndex].Term != args.PrevLogTerm {
		...
		k := args.PrevLogIndex
		for rf.log[k-1].Term == reply.ConflictTerm {
			k--
		}
		reply.ConflictIndex = k
		rf.mu.Unlock()
		reply.Success = false
		return
	}
...
func (rf *Raft) startElection() {
	...
	// DPrintf("**************************server %d win an election with log %v in term %d with votes %v", rf.me, rf.log, rf.currentTerm, voters)
		...
		go func(server int) {
			...
			if reply.Success == false {
				...
				if reply.ConflictTerm != 0 {
					k := 0
					for k < len(rf.log) && rf.log[k].Term != reply.ConflictTerm {
						k++
					}
					if k != len(rf.log) {
						find = true
						for k+1 < len(rf.log) && rf.log[k+1].Term == reply.ConflictTerm {
							k++
						}
						rf.nextIndex[server] = k
					}
				}
				if !find {
